...
class Rebel(CorpusParser):
    def __init__(self):
        super().__init__("/mnt/ssd2/corpora/corpora-thirdparty/corpus-rebel/rebel_dataset", test=True, dev=False)

        files = []

        if self.dev:
            files.append(os.path.join(self.data_dir, "en_val.jsonl"))
        if self.test:
            files.append(os.path.join(self.data_dir, "en_test.jsonl"))
        if self.train:
            files.append(os.path.join(self.data_dir, "en_train.jsonl"))
